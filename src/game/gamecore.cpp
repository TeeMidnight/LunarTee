...
void CCharacterCore::Tick(bool UseInput, const CTuningParams* pTuningParams)
{
	...
	if(m_HookState == HOOK_GRABBED)
	{
		if(m_HookedPlayer != -1)
		{
			CCharacterCore *pCharCore = m_pWorld->FindCharacter(m_HookedPlayer);
			if(pCharCore)
		...
		m_HookTick++;
		if(m_HookedPlayer != -1 && (m_HookTick > SERVER_TICK_SPEED+SERVER_TICK_SPEED/5 || !m_pWorld->FindCharacter(m_HookedPlayer)))
		{
			m_HookedPlayer = -1;
			m_HookState = HOOK_RETRACTED;
			m_HookPos = m_Pos;
